//============================================================================
// Name        : Fraction.cpp
// Author      : H. Ye
// Version     :
// Copyright   : N/A
// Description : see sec3 Description.txt
//============================================================================

#include <iostream>
using namespace std;

class Fraction {
	public:
	Fraction(int n, int d);
	Fraction add(Fraction a);
	Fraction sub(Fraction a);
	Fraction mult(Fraction a);
	Fraction divide(Fraction a);
	int getN();
	int getD();
	void displace();
	int comp(Fraction a);
	private:
	int numerator, denominator;
	int get_greatest_common_divisor(int n, int d);
};

Fraction::Fraction(int n = 0, int d = 1): numerator(n), denominator(d){}

int Fraction::get_greatest_common_divisor(int n, int d){
    int temp;
    int a = n;
    int b = d;
    if(a < b)
    {
        temp = a;
        a = b;
        b = temp;
    }
    while(b != 0)
    {
        temp = a % b;
        a = b;
        b = temp;
    }
    return a;
}

int Fraction::getN(){
	return numerator;
}

int Fraction::getD(){
	return denominator;
}

Fraction Fraction::add(Fraction a){
	int n, thisN, newD, newN, gcd;
	n = a.getN() * this->denominator;
	newD = a.getD() * this->denominator;
	thisN = this->numerator * a.getD();
	int c = this->comp(a);
	if (c == 0){
		Fraction newF(a.getN()*2,a.getD());
		return newF;
	}
	else{
		newN = thisN + n;
		gcd = get_greatest_common_divisor(newN, newD);
		newN /= gcd;
		newD /= gcd;
		Fraction newF(newN, newD);
		return newF;
	}
}

Fraction Fraction::sub(Fraction a){
	int n, thisN, newD, newN, gcd;
	n = a.getN() * this->denominator;
	newD = a.getD() * this->denominator;
	thisN = this->numerator * a.getD();
	int c = this->comp(a);
	if (c == 0){
		Fraction newF;
		return newF;
	}
	else{
		newN = abs(thisN - n);
		gcd = get_greatest_common_divisor(newN, newD);
		newN /= gcd;
		newD /= gcd;
		Fraction newF(newN, newD);
		return newF;
	}
}

Fraction Fraction::mult(Fraction a){
	int newD, newN, gcd;
	newN = a.getN() * this->numerator;
	newD = a.getD() * this->denominator;
	gcd = get_greatest_common_divisor(newN, newD);
	newN /= gcd;
	newD /= gcd;
	Fraction newF(newN, newD);
	return newF;
}

Fraction Fraction::divide(Fraction a){
	int newD, newN, gcd;
	newD = a.getD() * this->numerator;
	newN = a.getN() * this->denominator;
	gcd = get_greatest_common_divisor(newN, newD);
	newN /= gcd;
	newD /= gcd;
	Fraction newF(newN, newD);
	return newF;
}

int Fraction::comp(Fraction a){
	int n, thisN;
	n = a.getN() * this->denominator;
	thisN = this->numerator * a.getD();
	if(n > thisN)
		return 1;
	else if(n < thisN)
		return -1;
	else
		return 0;
}

void Fraction::displace(){
	cout << "fraction is: " << numerator << "/" << denominator << endl;
}

int main() {
	Fraction zero;
	zero.displace();
	Fraction a(2,5);
	Fraction b(3,7);
	Fraction add = a.add(b);
	Fraction sub = a.sub(b);
	Fraction mul = a.mult(b);
	Fraction div = a.divide(b);
	a.displace();
	b.displace();
	cout << "after adding, ";
	add.displace();
	cout << "after sub, ";
	sub.displace();
	cout << "after mult, ";
	mul.displace();
	cout << "after div, ";
	div.displace();
	return 0;
}
